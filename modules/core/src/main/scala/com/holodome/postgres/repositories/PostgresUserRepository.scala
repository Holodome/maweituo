...
import com.holodome.postgres.sql.codecs._
import com.holodome.domain.users._

object PostgresUserRepository {
  def make[F[_]: Async](xa: Transactor[F]): UserRepository[F] =
    new PostgresUserRepository(xa)
}

private final class PostgresUserRepository[F[_]: MonadCancelThrow](xa: Transactor[F]) extends UserRepository[F] {
