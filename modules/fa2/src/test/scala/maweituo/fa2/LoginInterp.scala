...
class LoginInterp extends ScalaDsl with EN:
  ...
  Then("""user send {string} request to \/login_verify""") { (string: String, string2: String) =>
    resp = client.use { _.verifyLogin(VerifyLoginDTO(email, password)) }.unsafeRunSync()
  }
  ...
  Then("""user send {string} request to \/verify_reset_password""") { (string: String, string2: String) =>
