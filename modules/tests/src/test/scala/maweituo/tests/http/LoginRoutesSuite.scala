...
import maweituo.http.EndpointsBuilder
...
object LoginRoutesSuite extends SimpleIOSuite with Checkers with HttpSuite:
  ...
  loginTest("/register and /login work as expected") { (users, auth, log) =>
    ...
    forall(registerRequestGen) { reg =>
      ...
      given EndpointsBuilder[IO] = EndpointsBuilder[IO](auth)
      val login               = AuthEndpoints[IO](auth).routes
      val register            = RegisterEndpoints[IO](users).routes
      for
